...
export default function UpdateTaskForm({ task = {}, onCancel }) {
  ...
  const {
    ...
    isSuccess,
    data,
  } = useUpdateTask(task.taskPoolId);
  useEffect(() => {
    if (isError || isSuccess) setTimeout(() => onCancel(), 1500);
  }, [isError, isSuccess]);
  const onSubmit = (data) => {
  ...
  return (
    <>
      {isError && (
        <Alert message="Failed to update Task. Please try again" type="error" />
      )}
      {isSuccess && <Alert message="Task updated successfully" />}
      <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
        <div className="space-y-4">
          {fields.map(({ name, label, type, required, rows, option }) => (
            <div key={name}>
